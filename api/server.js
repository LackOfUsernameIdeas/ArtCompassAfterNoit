...
io.on("connection", (socket) => {
  ...
  socket.on("hardwareData", (data) => {
    ...
    // Изпращане на данните към всички свързани клиенти, освен към изпращащия
    socket.broadcast.emit("hardwareDataResponse", data);
  });

  // Слушане на събитието 'dataDone' от клиента
  socket.on("dataDoneTransmitting", (data) => {
    console.log("Получено съобщение за завършване на трансфер на данни:", data);
    // Можете да изпратите потвърждение обратно на клиента или извършите други действия
    socket.broadcast.emit(
      "statusMessage",
      "Клиентът приключи с предаването на данни"
    );
  });
